...
:
 ...
 // Initialize the velocity gradient.
    ...
    (
        ...
        mesh_,
        dimensionedTensor("gradU_PISO",dimVelocity/dimLength,tensor::zero)
    ),
            ...
            forAll(bladePoints[i][j], k)
            {
                if(minDisCellID[i][j][k] != -1)
                {
                    ...
                    if (pointInterpType[i] == "linear")
                    {
                        ...
                        vector dU = dx & gradU_PISO[minDisCellID[i][j][k]];
...
{  
    ...
    forAll(bladeForce, i)
    {
    scalar sum_body_force_turbine=0.0;
    ...
    sum_body_force_turbine = sum_body_force_turbine+mag(bodyForce[sphereCells[i][m]]);
                        }
                    }
                }  
            }
