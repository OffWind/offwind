    ...
    public static class Bundler
    {
        ...
        private static string ProcessVirtualPathDefault(string virtualPath, BundleOptions options)
        {
            ...
            return VirutalPathCache.GetOrAdd(virtualPath, str =>
            {
                ...
                var ver = ConfigurationManager.AppSettings["ScriptsVersion"];
                path += path.IndexOf('?') == -1
                    ? "?v=" + ver : "&v=" + ver;
        ...
        public static MvcHtmlString Link(this HtmlHelper html, string rel, string href, object htmlAttributes = null, BundleOptions options = BundleOptions.Normal)
        {
            ...
            if (href.StartsWith("~"))
                href = href.Replace("~", GetPrefix(html));
        ...
        public static MvcHtmlString Css(this HtmlHelper html, string href, string media = null, BundleOptions options = BundleOptions.Normal)
        {
            return media != null
        ...
        public static MvcHtmlString Img(this HtmlHelper html, string src, string alt, string link = null, object htmlAttributes = null)
        {
            ...
            if (src.StartsWith("~"))
                src = src.Replace("~", GetPrefix(html));
        ...
        public static MvcHtmlString Js(this HtmlHelper html, string src, BundleOptions options = BundleOptions.Normal, string attr = null)
        {
            if (string.IsNullOrEmpty(src))
                return MvcHtmlString.Empty;
            if (src.StartsWith("~"))
            ...
            if (attr != null) tag.MergeAttribute(attr, string.Empty);
        ...
        private static string GetPrefix(HtmlHelper html)
        {
            //var url = html.ViewContext.HttpContext.Request.Url;
            //var prefix = Helper.GetUrlPrefix(url, WebConfigurationManager.AppSettings["RelativeUrl"]);
            //return prefix;
            return "";
        }
        ...
        public static MvcHtmlString RenderJsBundle(this HtmlHelper html, string bundlePath, BundleOptions options = BundleOptions.Normal)
        {
#if Release
            if (options== BundleOptions.Normal)
        ...
        public static string Url(this HtmlHelper html, string path)
        {
            if (string.IsNullOrEmpty(path))
                return string.Empty;
            if (path.StartsWith("~"))
                path = path.Replace("~", GetPrefix(html));
            return path;
        }
    }
}